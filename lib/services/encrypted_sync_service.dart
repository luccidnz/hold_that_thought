...
/// Enhanced SyncService that handles encrypted thoughts
class EncryptedSyncService {
  final SyncProvider _provider;
  final EncryptedThoughtRepository _repo;
  ...
  EncryptedSyncService({
    ...
    required EncryptedThoughtRepository repository,
  ...
  String _getAudioStoragePath(String thoughtId) {
    ...
    // Use a direct check if this is an async method issue
  ...
  Future<bool> _uploadThoughtWithRetries(Thought thought) async {
    ...
    while (attempts < 3) {
      ...
      try {
        ...
        String localAudioPath = thought.path;

        // If E2EE is enabled and the crypto service is armed, encrypt the audio file before upload
        if (isE2eeEnabled && _cryptoService.isArmed && !thought.path.endsWith('.enc')) {
          localAudioPath = await _repo.encryptAudioFile(thought);
        }

        final audioPath = _getAudioStoragePath(thought.id);
        await _provider.uploadAudio(localPath: localAudioPath, objectPath: audioPath);
        ...
        _ref.invalidate(encryptedThoughtsListProvider);
